...
module.exports.getFacturasCuotasCliente = function(codclien, year, callback) {
    var facturas = [];

    if (!conector.controlDatabaseCuotas()){
    ...
    var connection = conector.getConnectionCuotas();
...
var crearJsonCuotas = function(codclien, year, numfactu, letraser, callback) {
    ...
    var obj = 
        ...
        sql += " WHERE f.codclien = ? AND YEAR(f.fecfactu) = ?  AND f.numfactu = ? AND st.letraser = ?";
            ...
            var con = conector.getConnectionCuotas();
                con.query(sql, function (err, cab) {
                    ...
                    var con2 = conector.getConnectionCuotas();
