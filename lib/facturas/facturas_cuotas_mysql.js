...
module.exports.getFacturasCuotasCliente = async (codclien, year, empresaId) => {
    var facturas = [];
    let connection = null;
    var sql = "";
    try {
        ...
        sql += " WHERE f.codclien = ? AND YEAR(f.fecfactu) = ? AND f.intconta = 1";
        ...
        var array = [];
        if (process.env.CUOTAS_TIPO_FACT) {
            // array = JSON.parse(process.env.CUOTAS_TIPO_FACT);//convertimos el objeto en json
            array = process.env.CUOTAS_TIPO_FACT.split(',')
        }

        if (process.env.CUOTAS_TIPO_FACT && array.length > 0) {
...
module.exports.postPrepararCorreosCuotas = function (codclien, year, numfactu, letraser, informe, done) {


    crearJsonCuotas(codclien, year, numfactu, letraser, (err, obj) => {
        if (err) return done(err);
        if (obj) {
            crearPdfsCuotas(numfactu, informe, obj, (err, result) => {
                if (err) return done(err);
                done(null, result);
            });
        }
...
var crearJsonCuotas = function (codclien, year, numfactu, letraser, callback) {

    var facturas = [];

    /* if (!connector.controlDatabaseCuotas()){
        // no hay cuotas, devuelve lista de facturas vacía siempre
    ...
    sql += " ORDER BY f.fecfactu DESC, f.codtipom, f.numfactu, lf.codtipoa, lf.numalbar, lf.numlinea;";
    sql = mysql2.format(sql, [codclien, year, numfactu, letraser]);
    var con = connector.getConnectionCuotas();
    con.query(sql, function (err, cab) {
        ...
        var array2 = [];
        if (process.env.CUOTAS_TIPO_FACT) {
            array2 = JSON.parse(process.env.CUOTAS_TIPO_FACT);//convertimos el objeto en json
        }
        if (process.env.CUOTAS_TIPO_FACT && array2.length > 0) {
            sql2 += " AND ven.codtipom IN (" + connector.sqlInString(array) + ")";
        }
        sql2 = mysql2.format(sql2, [codclien, year, numfactu, letraser]);
        var con2 = connector.getConnectionCuotas();
...
var crearPdfsCuotas = function (numfactu, informe, obj, callback) {
