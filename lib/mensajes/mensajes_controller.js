...
// PostMensaje
// permite dar de alta un mensaje
router.post('/sendnew/adjuntos/:empresaId', async (req, res, next) => {
    try {
        let mensaje = await mensajesDb.postSendMensajeNewAdjuntos(req.body.mensaje, req.params.empresaId)
        res.json(mensaje);
    }  catch(error) {
        console.log("SENDNEW (ERR): ", error);
        next(res.status(500).send(error.message));
