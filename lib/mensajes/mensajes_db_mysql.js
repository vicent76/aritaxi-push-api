...
module.exports.postSendMensajeNew = async (mensaje, callback) => {
    // (1) Obtener la lista de destinatarios
...
var fnObtainPlayersIds = async (mensaje, empresaId) => {
    ...
    if (mensaje.fichero) {
        const result = await fnObtainPlayersIdsFromFichero(mensaje.fichero, connection);
        if(result.length == 0)  throw new Error('El archivo no se puede procesar')
    } else {
        if (mensaje.usuPush && mensaje.usuPush.length > 0) {
