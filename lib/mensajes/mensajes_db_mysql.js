...
module.exports.getMensaje = function(id, callback) {
    ...
    sql += " LEFT JOIN usuarios.usuarios AS ad ON ad.codclien = mens.administradorId";
...
var fnSendMessage = async(mensaje, parametros, playList) => {
    try {
        ...
        const client = new OneSignal.Client(parametros.appId, parametros.restApi, { apiRoot: 'https://onesignal.com/api/v1/notifications'});

        try {
            const response = await client.createNotification(options);
