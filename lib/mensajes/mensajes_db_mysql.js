...
module.exports.postSendMensajeNew = async (mensaje, empresaId) => {
    try {
        // (1) Obtener la lista de destinatarios
        const [res] = await fnObtainPlayersIdsFromMensaje(mensaje, empresaId)
        var playList = res;
        const paramas = await parametrosDb.getParametro(0, empresaId)
        var parametros = paramas;
        ...
        if(body) {
            if(Array.isArray(body)) {
                fnPutMensajeNew(mensaje, mensaje.mensajeId, mensaje.administradorId, empresaId);
                return null;
            } else {
                throw new Error(body)
            }
...
var fnPutMensajeNew = async (mensaje, id, administradorId, empresaId) => {
    ...
    try {
        ...
        sql += " m.texto = ?, m.asunto = ?";
        sql += " WHERE m.mensajeId = ? AND mu.mensajeId = ?;"
        sql = mysql2.format(sql, [administradorId, texto, asunto, id, id]);
        let result = await connection.query(sql);
        await connection.end();
        return null;
    } catch(error) {
