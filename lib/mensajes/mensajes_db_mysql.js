...
module.exports.getMensajesUsuario = async (codusu, empresaId) => {
    ...
    try {
        ...
        //buscamos primero los dias a mostrtar de los mensajes
        sql = "SELECT * FROM app_parametros WHERE parametroId = 0";
        sql = mysql2.format(sql);
        const [params] = await connection.query(sql);
        if (params.length > 0) {    
            dias = params[0].diasMostrar;
        }
        if (dias == 0 || dias == null) dias = 100000; // por defecto, si no hay un valor en la base de datos, mostramos los mensajes de los últimos 100000 días, es decir, todos los mensajes
        ...
        sql += " AND mu.estado != 'ARCHIVADO'";
        sql += " AND mu.fecha >= DATE_SUB(NOW(), INTERVAL ? DAY)";
        sql += " ORDER by m.fecha DESC";
        sql = mysql2.format(sql, [codusu, dias]);
...
module.exports.postSendMensajeNewAdjuntos = async (mensaje, empresaId) => {
    let res = []
    try {
        res = await fnObtainPlayersIds(mensaje, empresaId)
        var [playList] = res;
...
var fnStoreMensajeUsuarios = async (mensaje, playList, empresaId) => {
    ...
    try {
        let cfg = await connector.empresa(empresaId);
        connection = await mysql2.createConnection(cfg);
        sql = "INSERT INTO app_mensajes_usuariospush (mensajeId, usuarioPushId, fecha, estado) VALUES  ?";
...
var fnStoreMensajeUsuariosNew = async (mensaje, playList, empresaId) => {
    ...
    try {
        ...
        sql = "INSERT INTO app_mensajes_usuariospush (mensajeId, usuarioPushId, estado, fecha) VALUES  ?";
        sql = mysql2.format(sql, [records]);
        let [res] = await connection.query(sql);
        await connection.end();
        return res
