...
const estadisticas_alfa = {
    ...
    historicoServicios: async(desdeFecha, hastaFecha, pagina, pageSize) => {
        ...
        const url = `${alfaHistoricUrl}/historicaltrips?pageNumber=${pagina}&pageSize=${pageSize}&companyId=${alfaCompanyId}&fromDate=${desdeFecha}&toDate=${hastaFecha}`
    ...
    historicoServiciosPorEmpresa: async(desdeFecha, hastaFecha, pagina, empresa, pageSize) => {
        let alfaHistoricUrl = process.env.ALFA_URL
        let alfaCompanyId =  empresa
        let alfaUser =  process.env.ALFA_USER
        let alfaPassword =  process.env.ALFA_PASSWORD
        let alfaConfigurationId = process.env.ALFA_CONFIGURATIONID
        const url = `${alfaHistoricUrl}/historicaltrips?pageNumber=${pagina}&pageSize=${pageSize}&companyId=${alfaCompanyId}&fromDate=${desdeFecha}&toDate=${hastaFecha}`
