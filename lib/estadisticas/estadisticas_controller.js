...
router.get("/buscar-estadisticas/:dFecha/:hFecha", async(req, res, next) => {
    try {
        let desdeFecha = req.params.dFecha
        let hastaFecha = req.params.hFecha
        const result = await mysqlEstadisticas.getEstadisticas(dFecha, hFecha, empresaId)
        res.json(result)
    } catch (error) {
        next(error)
    }
