...
const estadisticas_mysql = {
    ...
    getEstadisticas6um: async (licencia) => {
        ...
        try {
            ...
            conn = await mysql.createConnection(cfg)
            let sql =`SELECT
             CONCAT(ELT(DATE_FORMAT(CONCAT(CAST(ano AS CHAR), '-', CAST(mes AS CHAR), '-', '1'),'%m'),'Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'),' ', CAST(ano AS CHAR)) as label,
