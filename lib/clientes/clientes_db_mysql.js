...
module.exports.getLogin = async (login, password, empresaId) => {
    ...
    try {
		let cfg = await connector.empresa(empresaId);
		connection = await mysql2.createConnection(cfg);
        sql = "SELECT s.*, c.nomchofe,";
        sql += " CASE";
        sql += "     WHEN LOCATE(',', s.nomclien) > 0 THEN TRIM(SUBSTRING_INDEX(s.nomclien, ',', -1))";
        sql += "     ELSE SUBSTRING_INDEX(s.nomclien, ' ', 1)";
        sql += " END AS formatNomclien";
        sql += " FROM sclien AS s";
        ...
        if (result && (result.length > 0)) {
            usuario = result[0];
            if (usuario.iban) {
                usuario.cuentaba = '*'.repeat(6) + usuario.cuentaba.slice(6);
                let compose = usuario.iban + usuario.codbanco.toString() + usuario.codsucur.toString() + usuario.digcontr.toString() + usuario.cuentaba
                usuario.iban = compose;
            }
            sql = `select sc2.* 
            from sclien_chofer as sc 
            left join schofe as sc2 on sc2.codchofe = sc.codchofe 
            where sc.codsocio = ${usuario.codclien} and (sc.fechabaj IS null OR sc.fechabaj ='0000-00-00')`
...
module.exports.getClientesBuscarLogadosTipo = async (q, empresaId, tipo) => {
  let conn = undefined;
  try {
    let cfg = await connector.empresa(empresaId);
    conn = await mysql2.createConnection(cfg);

    let sql = "SELECT * FROM sclien WHERE NOT playerId IS NULL AND fechabaj IS NULL";

    if (q && q !== "*") {
      switch(tipo) {
        case 'licencia':
          sql += " AND licencia LIKE ?";
          break;
        case 'codclien':
          sql += " AND codclien LIKE ?";
          break;
        case 'nomclien':
        default:
          sql += " AND nomclien LIKE ?";
      }
      sql = mysql2.format(sql, [`%${q}%`]);
    }
