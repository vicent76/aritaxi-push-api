...
router.get('/logados/tipo/:empresaId', async (req, res, next) => {
    try {
        const { q, tipo } = req.query; // q = texto, tipo = LICENCIA/NOMBRE/CODIGO
        let clientes = [];

        if (q) {
            clientes = await clientesDb.getClientesBuscarLogadosTipo(q, req.params.empresaId, tipo);
        } else {
            clientes = await clientesDb.getClientesLogados(req.params.empresaId);
        }
