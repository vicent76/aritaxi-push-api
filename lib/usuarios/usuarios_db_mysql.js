...
module.exports.getUsuarios = async (empresaId) =>{
	...
	try {
		...
		var sql = "SELECT * FROM usuarios WHERE nivelaritaxi >= 0";
...
module.exports.getUsuariosBuscar = async (nomusu, empresaId) => {
	...
	try {
		let cfg = await connector.usuarios(empresaId)
		conn = await mysql2.createConnection(cfg)
		var sql = "SELECT * FROM usuarios WHERE AND nivelaritaxi >= 0";
		if (nomusu !== "*") {
			sql = "SELECT * FROM usuarios WHERE nomusu LIKE ? AND nivelaritaxi >= 0";
